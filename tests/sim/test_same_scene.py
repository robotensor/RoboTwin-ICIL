...
def _build(task_env, seed):
    task_env.setup_demo(now_ep_num=0, seed=seed, is_test=True, **robotwin.SceneConfig().resolve())
    try:
...
def test_a_different_seed_is_a_different_scene():
    ...
    for seed in range(6):
        try:
            fingerprints.append(_build(task_env, seed))
        except robotwin.unstable_error():
            continue
