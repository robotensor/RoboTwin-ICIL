...
from robotwin_icil.policy import PolicyError, make_policy
from robotwin_icil.records import RunDir
from robotwin_icil.remote import RemotePolicy
from served_policies import EEReplay
...
@pytest.mark.parametrize("name", ["replay", "dummy"])
...
def test_a_remote_policy_is_described_and_named_as_the_policy_it_serves(tmp_path):
    """Including its action type: the client drives whichever path the server's policy needs,
    which it cannot know before it has said hello."""
    policy = RemotePolicy(policy="served_policies:EEReplay", log=tmp_path / "serve.log")
    try:
        assert policy.action_type == "ee" and policy.name == "ee_replay (remote)"
        assert policy.describe() == {**EEReplay().describe(), "remote": REMOTE}
    finally:
        policy.close()
...
def test_remote_is_a_built_in(tmp_path):
    ...
    try:
        ...
        policy.close()
    with pytest.raises(PolicyError, match="built-ins are dummy, remote, replay"):
        make_policy("nope")
