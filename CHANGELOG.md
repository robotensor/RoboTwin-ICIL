...
- (feat): a run chooses its robot. `eval` and `survey` take `--embodiment aloha-agilex` (one
  ...
  refuses to continue on another one. Verified on an RTX 5090: the click_bell replay scores 1/1 on
  both robots, seed 42 (aloha 60 steps, Franka 36 steps from a 45-frame, 16-wide demonstration),
  and the aloha record is unchanged (#81).
